...
const ComentariosModal = ({ showModal, toggleModal, inmuebleId }) => {
  ...
  useEffect(() => {
    if (showModal && inmuebleId) {
      fetch(`http://127.0.0.1:8000/api/getcomentarios/${inmuebleId}`)
        .then(response => {
          if (!response.ok) {
        ...
        .then(comentarios => {
          ...
          const promesasDeNombres = comentarios.map(comentario => {
            return fetch(`http://127.0.0.1:8000/api/getusuario/${comentario.idusuario}`)
  ...
  useEffect(() => {
    if (showModal) {
      fetch('http://127.0.0.1:8000/api/getcomentario/')
        .then(response => {
          if (!response.ok) {
            throw new Error('Network response was not ok');
          }
